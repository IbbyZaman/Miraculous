...
import { getFirestore, doc, getDoc, setDoc, deleteDoc, collection, getDocs, serverTimestamp } from "https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js";
...
window.MH_AUTH_READY = new Promise(resolve => {
  onAuthStateChanged(auth, user => {
    ...
    window.dispatchEvent(new CustomEvent("mh-auth-changed", { detail: user || null }));
    resolve(user || null);
  });
});
...
window.MH_AUTH = {
  ...
  async google() { return signInWithPopup(auth, provider); },
...
window.MH_CLOUD = {
  ...
  async savePlayback(key, data) {
    ...
    await setDoc(doc(db, "users", u.uid, "playback", key), { ...data, updatedAt: serverTimestamp() }, { merge: true });
  },
  async getFavourite(key) {
    const u = auth.currentUser; if (!u) return false;
    return (await getDoc(doc(db, "users", u.uid, "favourites", key))).exists();
  },
  async setFavourite(key, episode) {
    const u = auth.currentUser; if (!u) throw new Error("Sign in required");
    const ref = doc(db, "users", u.uid, "favourites", key);
    const snap = await getDoc(ref);
    if (snap.exists()) { await deleteDoc(ref); return false; }
    await setDoc(ref, { ...episode, addedAt: serverTimestamp() }); return true;
  },
  async markWatched(key, episode) {
    const u = auth.currentUser; if (!u) return;
    await setDoc(doc(db, "users", u.uid, "watched", key), { ...episode, watched: true, watchedAt: serverTimestamp() }, { merge: true });
  },
  async getWatched(key) {
    const u = auth.currentUser; if (!u) return false;
    return (await getDoc(doc(db, "users", u.uid, "watched", key))).exists();
  },
  async getAllPlayback() {
    const u = auth.currentUser; if (!u) return [];
    const snap = await getDocs(collection(db, "users", u.uid, "playback"));
